  ...
  export default function AcaraDetail() {
    ...
    const [galeri, setGaleri] = useState([]); // 🟢 TAMBAHAN
    ...
    useEffect(() => {
      ...
      const fetchEvent = async () => {
        ...
        try {
          ...
          // 🔹 2️⃣ Ambil galeri dari tabel galeri_acara
...
if (galeriError) {
  console.error("❌ Gagal ambil galeri:", galeriError);
} else {
  console.log("✅ Galeri acara:", galeriData);
}
    ...
    if (!event)
      return (
        <div className="min-h-screen flex flex-col justify-center items-center bg-gray-50 text-gray-700 p-8">
          <p className="text-xl font-medium mb-4">{errorMsg || "Acara tidak ditemukan 😢"}</p>
          <button
    ...
    // 🟢 Galeri diambil dari tabel galeri_acara, fallback kalau kosong
    ...
    const timList = Array.isArray(tim) && tim.length
      ? tim
      : (parsedPJ.length ? parsedPJ : [{ name: "Relawan", role: "Peserta Penanaman" }]);
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="bg-white rounded-2xl overflow-hidden border border-gray-200">
                <div className="w-full h-[300px] md:h-[500px] relative">
                  {/* gunakan <img> untuk external supabase url (lebih aman tanpa next.config) */}
                  <img
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {galeriList.map((imgSrc, i) => (
                    <div key={i} className="w-full aspect-square rounded-lg overflow-hidden border border-gray-100">
                      <img src={imgSrc || "/default.jpg"} alt={`Galeri ${i}`} className="object-cover w-full h-full" />
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-white p-8 md:p-10 rounded-2xl border border-gray-200">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {Array.isArray(timList) && timList.length > 0 ? (
                    timList.map((t, i) => {
                      // t bisa berupa object {name, role} atau string, handle keduanya
                      const name = t?.name ?? t?.fullName ?? (typeof t === "string" ? t : "Relawan");
                      const role = t?.role ?? t?.jabatan ?? "Peserta Penanaman";
                      return (
                        <div key={i} className="p-5 bg-green-50/50 border border-green-300 rounded-xl">
            ...
            <div className="lg:col-span-1 space-y-6 sticky top-8 h-fit">
              <div className="bg-green-100 p-8 rounded-2xl text-gray-800 border border-green-300">
                <div className="text-center">
                  ...
                  <button className="w-full bg-[#15803D] hover:bg-[#065F46] text-white font-bold text-lg px-6 py-3 rounded-xl">
              ...
              <div className="bg-white p-6 rounded-2xl border border-gray-200">
                ...
                <p className="text-sm text-gray-600 mb-2">
                  ID Acara: <span className="font-semibold text-gray-800">#{id}</span>
                </p>
                <p className="text-sm text-gray-600">
                  Tanggal: <span className="font-semibold text-gray-800">{tanggal || "-"}</span>
                </p>
                <p className="text-sm text-gray-600">
                  Lokasi: <span className="font-semibold text-gray-800">{lokasi || "-"}</span>
                </p>
              </div>
            </div>
          </div>
